import { validateAge, validateName, validatePostalCode, validateEmail, validateCity } from "./validator";
...
describe('validateName Unit Test Suits', () => {
  ...
  it.each([
    ...
    [`${"jav"}ascript:alert(1)`, 'javascript protocol'],
 ...
 * @description Tests for the validateCity function that validates city names
 */
describe('validateCity Unit Test Suits', () => {
  ...
  it('should accept city name with hyphen', () => {
    const result = validateCity('Saint-Denis');
    
    expect(result.valid).toBe(true);
    expect(result.sanitized).toBe('Saint-Denis');
  });

  it('should accept city name with multiple words', () => {
    const result = validateCity('La Rochelle');
    
    expect(result.valid).toBe(true);
    expect(result.sanitized).toBe('La Rochelle');
  });

  it('should accept city name with accented characters', () => {
    const result = validateCity('Aix-en-Provence');
    
    expect(result.valid).toBe(true);
    expect(result.sanitized).toBe('Aix-en-Provence');
  });

  it('should accept city with special French characters', () => {
    const result = validateCity('Séné');
    
    expect(result.valid).toBe(true);
    expect(result.sanitized).toBe('Séné');
  });

  it('should trim and accept city with spaces', () => {
    const result = validateCity('  Lyon  ');
    
    expect(result.valid).toBe(true);
    expect(result.sanitized).toBe('Lyon');
  });

  // Cas invalides : chiffres
  it('should reject city name with numbers', () => {
    const result = validateCity('Paris75');
    
    expect(result.valid).toBe(false);
    expect(result.error.code).toBe('INVALID_CITY_FORMAT');
    expect(result.error.message).toContain('cannot contain numbers');
  });

  it('should reject city name with only numbers', () => {
    const result = validateCity('75001');
  ...
  // Cas invalides : caractères spéciaux
  it('should reject city name with special characters', () => {
    const result = validateCity('Paris@City');
  ...
  it('should reject city name with underscore', () => {
    const result = validateCity('Saint_Denis');
  ...
  // Protection XSS
  it('should detect XSS attack with HTML tags', () => {
    const result = validateCity('<script>alert("xss")</script>');
    
    expect(result.valid).toBe(false);
    expect(result.error.code).toBe('XSS_DETECTED');
    expect(result.error.message).toContain('XSS attack');
  });

  it('should detect XSS attack with javascript protocol', () => {
    const xssAttempt = `${"jav"}ascript:alert(1)`;
    const result = validateCity(xssAttempt);
    
    expect(result.valid).toBe(false);
    expect(result.error.code).toBe('XSS_DETECTED');
  });

  it('should detect XSS attack with img tag', () => {
    const result = validateCity('<img src=x onerror=alert(1)>');
    
    expect(result.valid).toBe(false);
    expect(result.error.code).toBe('XSS_DETECTED');
  });

  // Cas invalides : valeurs manquantes
  it('should return error for empty string', () => {
  ...
  it('should return error for undefined', () => {
    const result = validateCity(undefined);
  ...
  // Cas limites
  it('should reject city with only spaces', () => {
    const result = validateCity('     ');
